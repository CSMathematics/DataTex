    ...
    ui(new Ui::SolveDatabaseExercise)
{
    ...
    connect(ui->SolutionContent->editor,&QTextEdit::textChanged,this,[=](){ui->SolutionContent->toolBar->Save->setEnabled(true);});
...
//    ui->scrollArea->setMinimumWidth(200);
    ...
    connect(ui->SolutionReady,&QPushButton::toggled,this,[&](bool checked){
        ...
        setSolutionState.exec(QString("UPDATE Database_Files SET Solved_Prooved = '%1' WHERE Id = '%2'").arg(QString::number(checked),ui->SolutionsCombo->currentText()));
//        int s = SqlFunctions::Get_String_From_Query(SqlFunctions::GetSolutionState.arg(QFileInfo(Exercise).baseName()),currentbase).toInt();
        setSolutionState.exec(QString("UPDATE Database_Files SET Solved_Prooved = '%1' WHERE Id = '%2'").arg(QString::number(checked),QFileInfo(Exercise).baseName()));
    ...
    QStringList Database_HeaderNames = SqlFunctions::Get_StringList_From_Query("SELECT Name FROM BackUp WHERE Table_Id = 'Metadata'",DataTex::CurrentTexFilesDataBase);
    QSqlQueryModel * Model = new QSqlQueryModel(this);
    QSqlQuery tableQuery(currentbase);


    QString query = SqlFunctions::ShowAllFiles_SELECT;
    query += "WHERE ft.Solvable >0 GROUP BY df.Id ORDER BY df.rowid;";
...
//        DBFileInfo fileInfo = DBFileInfo(ui->SolutionsCombo->currentText(),currentbase);
...
void SolveDatabaseExercise::ExerciseTable_SelectionChanged()
{
    if(ExerciseTable->currentIndex().row()>-1){
        ...
        FileType = ExerciseTable->model()->data(ExerciseTable->model()->index(row,ExerciseTable->model()->columnCount()-3)).toString();
...
//    QString SolutionName = QFileInfo(NewSolution).baseName();
    if(ExerciseTable->model()->rowCount()>0){
        DBFileInfo * solInfo = new DBFileInfo(ExerciseName,currentbase);//
...
//                                   "WHERE File_Id = '%2' ").arg(SolutionName,(ExerciseName)));
        ...
        DBFileInfo::WriteDBFile(solInfo);
...
void SolveDatabaseExercise::RebuildSolution()
{
    ...
    FileCommands::BuildDocument(DataTex::LatexCommands[CurrentBuildCommand],file,DataTex::LatexCommandsArguments[CurrentBuildCommand],".tex");
