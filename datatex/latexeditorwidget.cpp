...
void LatexTextBrowser::lineNumberAreaPaintEvent(QPaintEvent *event)
{
    ...
    QColor OtherLinesColor;
    if(isThemeDark){
        backgroundColor = QColor(125,125,125);
        CurrentLineColor = QColor(255,255,255);
        OtherLinesColor = QColor(240, 240, 240);
    }
    else{
        backgroundColor = QColor(230,230,230);
        CurrentLineColor = QColor(0,0,0);
        OtherLinesColor = QColor(120, 120, 120);
    }
...
//    QColor col_1(255,255,255);      // Current line (custom green)
//    QColor col_0(240, 240, 240);    // Other lines  (custom darkgrey)
    QFont Bfont;
    Bfont.setBold(true);
    QFont font;
    font.setBold(false);
    ...
    while (block.isValid() && top <= event->rect().bottom()) {
        if (block.isVisible() && bottom >= event->rect().top()) {
            ...
            painter.setPen(OtherLinesColor);
...
void LatexTextEdit::SetEditorReadOnly(LatexTextEdit * widget)
{
    ...
    if(CurrentLineText.startsWith("%# File Id") || CurrentLineText.startsWith("%# End of file")
        || CurrentLineText.startsWith("%# Document Id :") || CurrentLineText.startsWith("%@ FilesDB Id :")
        || CurrentLineText.startsWith("%@ DocumentsDB Id :")){
...
FileToolBar::FileToolBar(LatexTextWidget *editor,bool exclusiveTexPdf) : QToolBar(editor)
{
    ...
    ShowTex = new QAction(QIcon::fromTheme("LatexConfig"),"", this);
    ...
    ShowPdf = new QAction(QIcon::fromTheme("Pdf2"),"", this);
    ShowPdf->setCheckable(true);
    connect(ShowPdf,&QAction::triggered,editor,[=](){emit showPdfFile();});

    ShowMetadata = new QAction(QIcon::fromTheme("HelpAbout"),"", this);

    BuildFile = new QAction(QIcon::fromTheme("MediaPlaybackStart"),"", this);
    connect(BuildFile,&QAction::triggered,editor,[=](){emit Build();});
    OpenFolder = new QAction(QIcon::fromTheme("DocumentOpenData"),"", this);
    ...
    ShowTopBar = new QAction(QIcon::fromTheme("topbar"),"", this);
    ...
    ShowSideBar = new QAction(QIcon::fromTheme("sidebar"),"", this);
