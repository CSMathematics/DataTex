...
numLeftPar=0;
s=text; 
tagStart=s.indexOf(QRegularExpression("\\\\"+struct_level3+"\\*?[\\{\\[]"), tagEnd);
tagEnd=-1;
offset=tagStart;
	if ((tagStart!=-1) && ((tagComment==-1) || ((tagComment!=-1) && (tagStart<=tagComment))))
	{
...
numLeftPar=0;
s=text;
tagStart=s.indexOf(QRegularExpression("\\\\"+struct_level4+"\\*?[\\{\\[]"), tagEnd);
tagEnd=-1;
offset=tagStart;
if ((tagStart!=-1) && ((tagComment==-1) || ((tagComment!=-1) && (tagStart<=tagComment))))
	{
...
numLeftPar=0;
s=text;
tagStart=s.indexOf(QRegularExpression("\\\\"+struct_level5+"\\*?[\\{\\[]"), tagEnd);
tagEnd=-1;
offset=tagStart;
if ((tagStart!=-1) && ((tagComment==-1) || ((tagComment!=-1) && (tagStart<=tagComment))))
	{
...
tagStart=tagEnd=offset=0;
s=text;
tagStart=s.indexOf(QRegularExpression("\\\\begin\\{block\\}\\*?[\\{\\[]"), tagEnd);
offset=tagStart;
if ((tagStart!=-1) && ((tagComment==-1) || ((tagComment!=-1) && (tagStart<=tagComment))))
	{
...
numLeftPar=0;
s=text;
tagStart=s.indexOf(QRegularExpression("\\\\"+struct_level1+"\\*?[\\{\\[]"), tagEnd);
tagEnd=-1;
offset=tagStart;
if ((tagStart!=-1) && ((tagComment==-1) || ((tagComment!=-1) && (tagStart<=tagComment))))
	{
...
numLeftPar=0;
s=text;
tagStart=s.indexOf(QRegularExpression("\\\\"+struct_level2+"\\*?[\\{\\[]"), tagEnd);
tagEnd=-1;
offset=tagStart;
if ((tagStart!=-1) && ((tagComment==-1) || ((tagComment!=-1) && (tagStart<=tagComment))))
	{
...
QJSValue appContext = fScriptEngine.newQObject(this);
...
#else
setTabStopDistance(fontMetrics().horizontalAdvance(" ")*tabWidth);
#endif
                ...
                if (relativePos < line.textLength() - line.textStart()) {
                    ...
                    if (doc->characterAt(context.cursorPosition) == QLatin1Char('\t')) {
                        doSelection = false;
                            ...
                            #else
                            qreal space = QFontMetricsF(layout->font()).horizontalAdvance(QLatin1Char(' '));
                            #endif
                        if (w > space) {
                            x += w-space;
                            w = space;
                            ...
                            #else
                            w = QFontMetrics(layout->font()).horizontalAdvance(QLatin1Char(' ')); // in sync with QTextLine::draw()
                            #endif
...
if (inlinecheckSpelling && pChecker && !textCursor().hasSelection())
	...
	cols=colstart;
		...
		if (!ignoredwordList.contains(word) && !hardignoredwordList.contains(word))
            ...
            check = false;/*pChecker->spell(encodedString.data());*/
			if (!check)
				{
...
QTextCursor found = isRegExp?document()->find(QRegularExpression(expr), c, flags) : document()->find(expr, c, flags);
...
QTextCursor found = isRegExp?document()->find(QRegularExpression(expr), start, flags) : document()->find(expr, start, flags);
...
if (c.hasSelection()) 
	...
	if (isRegExp)
	  {
	  QRegularExpression expr1(o);
	  QStringList texts;
	  QString rtext1=r;
	  int pos = 0;
        while (pos != -1)
	    {
        QRegularExpressionMatch expr1Match=expr1.match(c.selectedText(),pos);
        if (expr1Match.hasMatch())
            {
            texts << expr1Match.captured(0);
            pos=expr1Match.capturedStart();
            pos += expr1Match.capturedLength(0);
            }
        else pos=-1;
	    }
...
{
QFileInfo fi(fname);
bool asy =(!fname.startsWith("untitled") && (fi.suffix()=="asy")) ;
bool go=true;
...
while (start > 0) 
  ...
  if (!isControlSeq && (ch == '\'' || ch == QChar(0x2019)) && start > 0 && IS_WORD_BACK(text.at(start - 1)))
    {
    includesApos = true;
    continue;
    }
...
while (++end < text.length()) 
  ...
  if (!isControlSeq && (ch == '\'' || ch == QChar(0x2019)) && end < text.length() - 1 && IS_WORD_FORWARD(text.at(end + 1)))
  {
  includesApos = true;
  continue;
  }
...
while (start > 0) 
  ...
  if (!isControlSeq && (ch == '\'' || ch == QChar(0x2019)) && start > 0 && IS_WORD_FORMING(text.at(start - 1)))
    {
    includesApos = true;
    continue;
    }
...
while (++end < text.length()) 
  ...
  if (!isControlSeq && (ch == '\'' || ch == QChar(0x2019)) && end < text.length() - 1 && IS_WORD_FORMING(text.at(end + 1)))
  {
  includesApos = true;
  continue;
  }
...
if (!ctrlOrShift && (e->key() == Qt::Key_Insert))
  ...
  setOverwriteMode(overmode);
	...
	if (txt.contains(QString(QChar(0x2022))))
	    {
	    //e->ignore();
	    search(QString(QChar(0x2022)) ,true,false,true,true,false);
	    return;		
	    }
	}
	...
	if (txtnext.contains(QString(QChar(0x2022))))
	    {
	    //e->ignore();
	    search(QString(QChar(0x2022)) ,true,false,true,true,false);
	    return;		
	...
	if (txt.contains(QString(QChar(0x2022))))
	    {
	    //e->ignore();
	    search(QString(QChar(0x2022)) ,true,false,false,true,false);
	    return;		
	    }
	}
	...
	if (s1.matches(vfocus)==QKeySequence::ExactMatch) emit requestpdf(textCursor().blockNumber() + 1);
	  ...
	  if (block.isValid()) 
		  ...
		  cursor.setPosition(endw, QTextCursor::KeepAnchor);
            ...
            while (index != -1)
			{
            QRegularExpressionMatch rxMatch=rx.match(code,index);
                if (rxMatch.hasMatch())
                {
                if (rxMatch.captured(0)=="@") code.replace(rxMatch.capturedStart(0),1,QString(QChar(0x2022)));
                else if (rxMatch.captured(0)=="@@") code.replace(rxMatch.capturedStart(0),2,"@");
                index=rxMatch.capturedStart(0);
                index += rxMatch.capturedLength(0);
                }
            else index=-1;
			}
		      //code.replace("@",QString(QChar(0x2022)));
		      if (code.left(1)=="%")
			{
			...
			code="\\begin{"+code+"}\n"+QString(QChar(0x2022))+"\n\\end{"+code+"}\n";
		      ...
		      //replace(code,false,"");
		      //int pos=cursor.position();
		      if (code.contains(QString(QChar(0x2022))))
			{
...
QString env;
QRegularExpression rbb("begin\\{\\s*([A-Za-z_]+(\\*)?)\\}");
QRegularExpressionMatch rbbMatch=rbb.match(completion);
//QRegExp rbb("begin\\{\\s*(.*)\\}");
if (rbbMatch.hasMatch())
	{
	tc.insertText(insert_word);
	insertNewLine();
	insertNewLine();
	tc.select(QTextCursor::WordUnderCursor);
	env=rbbMatch.captured(1);
	tc.insertText("\\end{"+env+"}");
	tc.movePosition(QTextCursor::Up,QTextCursor::MoveAnchor,1);
	setTextCursor(tc);
	if (insert_word.contains(QString(QChar(0x2022))))
	    {
	    tc.movePosition(QTextCursor::Up,QTextCursor::MoveAnchor,1);
	    setTextCursor(tc);
	    search(QString(QChar(0x2022)) ,true,false,true,true,false);
	...
	else if (!search(QString(QChar(0x2022)) ,true,false,true,true,false))
	  {
	  tc.setPosition(pos+completion.length(),QTextCursor::MoveAnchor);
	  setTextCursor(tc);
	  }
...
QRegularExpression rxpics("^file:(/+)(.*\\.(?:eps|pdf|png|jpeg|jpg|tiff))$");
#else
QRegularExpression rxpics("^file:(//)(.*\\.(?:eps|pdf|png|jpeg|jpg|tiff))$");
#endif
...
cursor.removeSelectedText();
QRegularExpressionMatch rxpicsMatch=rxpics.match(uri);
if (rxpicsMatch.hasMatch())
  {
  fn=rxpicsMatch.captured(2);
...
if ( !location.isEmpty() ) 
	...
	if (scriptfile.open(QFile::ReadOnly))
        ...
        #else
        ts_script.setEncoding(QStringConverter::Encoding::Utf8);
        #endif
	    text= ts_script.readAll();
	    }
QJSValue result = fScriptEngine.evaluate(text);
	}
}
