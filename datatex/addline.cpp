...
PreambleSettings::PreambleSettings(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::PreambleSettings)
{
    ...
    textEdit = new LatexTextWidget(this,false,true);
    ...
    textEdit->editor->setText("\\documentclass[a4paper,10pt]{<Select document type>}\n");
    QTextCursor cursor = textEdit->editor->textCursor();
    cursor.movePosition(QTextCursor::End);
    textEdit->editor->setTextCursor(cursor);
    ...
    connect(ui->BabelList, &QListWidget::itemClicked,this,[&](QListWidgetItem *item){
        ...
        QString text = textEdit->editor->toPlainText();
        text.replace(temp_LanguageLine,LanguageLine);
        textEdit->editor->setText(text);
        getLanguage();
    });
    ...
    connect(ui->BabelPolyglossiaCheckBox,&QCheckBox::toggled,this,[&](bool checked){
        ...
        if(!getLanguage()){
            textEdit->editor->insertPlainText("\\usepackage["+Languages.join(",")+"]{babel}\n");
        }
        else{
            QString text = textEdit->editor->toPlainText();
    ...
    connect(ui->TamplateTree,&QTreeWidget::itemClicked,this,[&](QTreeWidgetItem * item){
        ...
        if(resBtn == QMessageBox::Yes){
            textEdit->editor->setText(item->text(1));
        }
        else{
            return;
        }
    ...
    textEdit->toolBar->Save->setVisible(false);
}
...
{
    QStringList Line = {ui->NameLine->text(),ui->CodeLine->text(),textEdit->editor->toPlainText()};
...
void PreambleSettings::on_AddButton_clicked()
{
    ...
    textEdit->editor->insertPlainText("\\usepackage{"+package+"}\n");
}
...
void PreambleSettings::on_textEdit_cursorPositionChanged()
{
    QTextCursor cursor = textEdit->editor->textCursor();
    int row = cursor.blockNumber();
}
