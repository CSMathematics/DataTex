...
//    }
    ...
    ui->verticalLayout_9->addWidget(TheoryView);
    TheoryView->setMinimumWidth(320);
    TheoryView->show();

    ExerciseView = new PdfViewer(this);
    ui->verticalLayout_5->addWidget(ExerciseView);
    ExerciseView->setMinimumWidth(320);
    ExerciseView->show();

    CombExerciseView = new PdfViewer(this);
    ui->verticalLayout_2->addWidget(CombExerciseView);
    CombExerciseView->setMinimumWidth(320);
    CombExerciseView->show();

    ui->splitter_2->setSizes(QList<int>({1, 700}));
    ui->splitter_4->setSizes(QList<int>({200, 1}));
    ui->splitter_5->setSizes(QList<int>({500, 1}));
}

void NewDatabaseFile::InitialSettings()
{
    QSqlQuery InitialValuesQuery(DataTex::DataTeX_Settings);
...
//    ui->FileTable->horizontalHeader()->setSizeAdjustPolicy(QAbstractScrollArea::AdjustToContents);
    ...
    connect(ui->FileTable->selectionModel(), &QItemSelectionModel::selectionChanged,this, &NewDatabaseFile::FileTable_selection_changed);
}
...
//    ui->TheoryFileTypeMetadataLine->setText(btn->text().remove("&"));
...
void NewDatabaseFile::FileTable_selection_changed()
{
    ...
    QString FilePath = ui->FileTable->model()->data(ui->FileTable->model()->index(row,2)).toString();
...
//    ui->FileTypeMetadataLine->setText(btn->text().remove("&"));
...
//    ui->FileTypeMetadataLine->setText(exer_sub);
    ...
    mapIdsNames.insert("Difficulty","0");
    mapIdsNames.insert("Date",QDateTime::currentDateTime().toString("dd/M/yyyy hh:mm"));
    mapIdsNames.insert("Id",QFileInfo(ui->FileNameLine->text()).baseName());
    mapIdsNames.insert("Path",ui->FilePathLine->text()+ui->FileNameLine->text());
...
void NewDatabaseFile::ExerciseFileList_selection_changed()
{
    ...
    QString outName = QFileInfo(FilePath).path() + QDir::separator()+ QFileInfo(FilePath).baseName()+".pdf";
    if(QFileInfo::exists(outName)){
        DataTex::loadImageFile(outName,ExerciseView);
    }
}
...
void NewDatabaseFile::on_OkDialogButton_rejected()
{
    ...
    QSqlQuery SetInitialValues(DataTex::DataTeX_Settings);
...
void NewDatabaseFile::TableItemDoubleClicked(QTableView * table,const QModelIndex &index,int column)
{
    ...
    else{DataTex::CreateTexFile(FilePath);QDesktopServices::openUrl(QUrl("file:///"+FilePath));}
}
