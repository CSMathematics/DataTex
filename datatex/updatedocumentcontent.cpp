    ...
    ui(new Ui::UpdateDocumentContent)
{
    ...
    file.close();
    ui->CurrentDocumentContent->setPlainText(Content);
    ...
    ui->UpdatedDocumentContent->setPlainText(Content);

    QString f = "(\""+fileNames.join("\",\"")+"\")";
    QSqlQueryModel * Files = new QSqlQueryModel(this);
    QStringList datalist = {SqlFunctions::ShowFilesInADocument.arg(f,QFileInfo(DataTex::CurrentDataBasePath).baseName())};
    QString query;
...
//    LatexTextEdit::ShowButtons(ui->UpdatedDocumentContent);
    connect(ui->CurrentDocumentContent->verticalScrollBar(),&QScrollBar::valueChanged,this,[=](){
        ui->UpdatedDocumentContent->verticalScrollBar()->setValue(ui->CurrentDocumentContent->verticalScrollBar()->value());
    });
    connect(ui->UpdatedDocumentContent->verticalScrollBar(),&QScrollBar::valueChanged,this,[=](){
        ui->CurrentDocumentContent->verticalScrollBar()->setValue(ui->UpdatedDocumentContent->verticalScrollBar()->value());
    });
...
UpdateDocumentContent::~UpdateDocumentContent()
{
    ...
    for (int i=0;i<DatabasesInDocument.count();i++) {
        if(DatabasesInDocument[i].databaseName()!=DataTex::CurrentDataBasePath){
...
{
    QString DocContent = ui->CurrentDocumentContent->toPlainText();
    int lineNumber = -1;
    QStringList Lines;
    QString FileText;
    ...
    while(!parse.atEnd()){
        ...
        if(line.startsWith("%# End of file")){
            readEnabled = false;
            FileText += Lines.join("\n");
            FilesContentsFormDocument.insert(FilesInDocument[lineNumber],FileText);
            Lines.clear();
            FileText.clear();
        }
    }
