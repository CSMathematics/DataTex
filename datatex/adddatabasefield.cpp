    ...
    connect(ui->CodeLine,&QLineEdit::textChanged,this,[&](QString text){
        ...
        if(IsSubSection){
            if(MapSubSecIds.keys().contains(text)){
                ...
                idAction = ui->CodeLine->addAction(QIcon::fromTheme("DocumentEncrypted"), QLineEdit::LeadingPosition);
            }
            else{
    ...
    connect(ui->NameLine,&QLineEdit::textChanged,this,[&](QString text){
        ...
        if(IsSubSection){
            if(MapSubSecNames.keys().contains(text)){
                ...
                nameAction = ui->NameLine->addAction(QIcon::fromTheme("DocumentEncrypted"), QLineEdit::LeadingPosition);
            }
            else{
