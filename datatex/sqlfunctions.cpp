...
const QString SqlFunctions::SelectCurrentFilesDatabase =
                        "SELECT Value FROM Current_Databases WHERE Setting = \"Current_FilesDB\"";

const QString SqlFunctions::SelectCurrentDocumentsDatabase =
                        "SELECT Value FROM Current_Databases WHERE Setting = \"Current_DocumentsDB\"";
...
const QString SqlFunctions::CountFiles_by_FileType =
                        "SELECT FileTypes.Name AS 'File type',COUNT(*) AS 'Number' "
...
//                            "SELECT DISTINCT df.Id, \"%2\" AS \"Database source\" ,ft.Name, "
...
{
    ShowAllFiles_SELECT = "SELECT df.Id ,ft.Name "
...
void SqlFunctions::FilterTable(QStringList fields,QStringList values,QStringList additionalFilters)
{
    ...
    if(!values.at(1).isEmpty() && !values.at(1).isNull()){
        DataFields.append(" ft.Name LIKE \"%"+values.at(1)+"%\" ");
    }
