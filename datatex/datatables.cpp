...
void DataTables::LoadFields()
{
    ...
    while(fields.next()){
        ...
        ui->FieldTable->setItem(item,0 , new QTableWidgetItem(fields.value(1).toString()));
        ui->FieldTable->setItem(item,1 , new QTableWidgetItem(fields.value(0).toString()));
        ui->ComboFields_ChapterTab->addItem(fields.value(1).toString(), QVariant(fields.value(0).toString()));
        ui->ComboFields_SectionTab->addItem(fields.value(1).toString(), QVariant(fields.value(0).toString()));
        ui->ComboFields_ExerciseTypeTab->addItem(fields.value(1).toString(), QVariant(fields.value(0).toString()));
...
void DataTables::LoadFileTypes()
{
    ...
    FileTypes.exec("SELECT \"Name\",\"Id\",\"FolderName\" FROM \"FileTypes\"");
    int i=-1;
...
void DataTables::AddField(QStringList Line)
{
    ...
    if(AddField.exec()){
        ...
        ui->FieldTable->setItem(i,0 , new QTableWidgetItem(Line[1]));
        ui->FieldTable->setItem(i,1 , new QTableWidgetItem(Line[0]));
        ui->ComboFields_ChapterTab->addItem(Line[0],QVariant(Line[0]));
        ui->ComboFields_ChapterTab->setCurrentIndex(-1);
        ui->ComboFields_SectionTab->addItem(Line[0],QVariant(Line[0]));
        ui->ComboFields_SectionTab->setCurrentIndex(-1);
        ui->ComboFields_ExerciseTypeTab->addItem(Line[0],QVariant(Line[0]));
        ui->ComboFields_ExerciseTypeTab->setCurrentIndex(-1);
    }
    else{
...
{
    QString field = ui->ComboFields_ChapterTab->currentText();
    ...
    int i=-1;
    ChaptersQuery.exec(SqlFunctions::Chapters_Query.arg(field));
...
void DataTables::on_RemoveSectionButton_clicked()
{
    ...
    QString chapter = ui->ComboChapters_SectionTab->currentData().toString();
    QMessageBox::StandardButton resBtn = QMessageBox::question( this,
    ...
    if (resBtn == QMessageBox::Yes) {
        ...
        RemoveSection.exec(QString("DELETE FROM Sections WHERE Id = \"%1\" AND Chapter = \"%2\"")
                           .arg(code,chapter));
...
void DataTables::UpdateDatabaseMetadata(QString Id, QString DBField, QString oldId1, QString newId1, QString oldPath1, QString newPath1, QString oldPath2, QString newPath2)
{
    ...
    for(int i=0;i<Ids.count();i++){
        ...
        text.replace("%# File Id : "+oldId,"%# File Id : "+id);
...
//    QStringList Names = SqlFunctions::Get_StringList_From_Query("SELECT Name FROM FileTypes",currentbase);
...
//        if(NewFileType.exec()){
            ui->FileTypeTable->insertRow(i);
            ui->FileTypeTable->setItem(i,0 , new QTableWidgetItem(filetype.Id));
            ui->FileTypeTable->setItem(i,1 , new QTableWidgetItem(filetype.Name));
            ui->FileTypeTable->setItem(i,2 , new QTableWidgetItem(filetype.FolderName));
//        }
//        else{
