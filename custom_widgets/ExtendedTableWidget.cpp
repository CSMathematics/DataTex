...
void ExtendedTableWidget::useAsFilter(const QString& filterOperator, bool binary, const QString& operatorSuffix)
{
    ...
    if (!index.isValid() || !selectionModel()->hasSelection()/* || m->isBinary(index)*/)
        return;
    QVariant bArrdata = model()->data(index, Qt::DisplayRole/*EditRole*/);
    QString value;
    if (bArrdata.isNull())
        value = "NULL";
...
void ExtendedTableWidget::selectTableLine(QString text, int column)
{
    QAbstractItemModel * m = model();
    QModelIndex ix = currentIndex();
    while (model()->canFetchMore(ix))
        model()->fetchMore(ix);
    QModelIndexList matchList = m->match(m->index(0,column), Qt::DisplayRole,
                                         text, -1,  Qt::MatchFlags(Qt::MatchContains|Qt::MatchWrap));

    if(matchList.count()>=1){
        setCurrentIndex(matchList.first());
        scrollTo(matchList.first());
    }
}
...
void ExtendedTableWidget::filterTable(QString query, QSqlDatabase database,bool isSorted)
{
    FilesModel = new QSqlQueryModel(this);
    FilesProxyModel = new QSortFilterProxyModel(this);//Φιλτράρισμα και ταξινόμηση ---
    FilesProxyModel->setSourceModel(model());//Φιλτράρισμα και ταξινόμηση ---
    ...
    if(isSorted){
        setModel(FilesProxyModel);// Φιλτράρισμα και ταξινόμηση ---Filtering
    }
    else{
        setModel(FilesModel);// Φιλτράρισμα και ταξινόμηση ---Filtering
    }
    DatabaseQuery.last();
    int row_count = DatabaseQuery.at() + 1;
    emit filesfound(row_count);
    setSortingEnabled(isSorted);//Φιλτράρισμα και ταξινόμηση ---
}

//TableToolBar::TableToolBar(ExtendedTableWidget *table) : QWidget(table)
//{
//    Sort = new QPushButton(QIcon::fromTheme("SortPresence"),QString(),table);
//    ClearFilters = new QPushButton(QIcon::fromTheme("KtRemoveFilters"),QString(),table);

//    connect(Sort,&QPushButton::toggled,this,[](bool checked){
//        table->filterTable(query,database,isSorted);
//    });

//    connect(ClearFilters,&QPushButton::toggled,this,[](bool checked){
//        //
//    });
//}
